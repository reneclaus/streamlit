...
export function PlotlyChart({
  ...
  if ("on_export" in plotlyConfig && plotlyConfig["on_export"]) {
    var icon1 = {
      'width': 500,
      'height': 600,
      // https://fontawesome.com/search?q=save&o=r
    ...
    plotlyConfig.modeBarButtonsToAdd = [
      {
        name: 'Export',
        icon: icon1,
        click: useCallback(function (gd: any) {
          widgetMgr.setStringValue(
            element,
            JSON.stringify({"on_export":true}),
            { fromUi: true },
            fragmentId
          )
          // We clear the on_export so that it doesn't get triggered on every rerun.
          widgetMgr.setStringValue(
            element,
            JSON.stringify({"on_export":false}),
            { fromUi: false },
            fragmentId
          )
        }, [element.id, widgetMgr, fragmentId])
